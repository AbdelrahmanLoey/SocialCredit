...
router.post('/forgot-password',async(req,res)=>{
  try{
    ...
    // verify() catches bad credentials immediately before trying to send
    await transport.verify()
    await transport.sendMail({
      from:`"Social Credit System" <${process.env.SMTP_USER}>`,
      to:email,
      subject:'☭ Password Reset Request',
      html:`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#090909;font-family:monospace}</style></head><body><table width="100%" style="background:#090909;min-height:100vh"><tr><td align="center" style="padding:40px 20px"><table width="560" style="max-width:560px"><tr><td style="background:#0d0a0d;border:2px solid #5a3a4a;padding:0"><table width="100%"><tr><td style="padding:6px;background:#0a070a"><table width="100%" style="border:1px solid #3a1f2a"><tr><td style="padding:6px;background:#090609"><table width="100%" style="border:1px dashed #2a1520"><tr><td align="center" style="padding:36px 40px 28px"><p style="font-size:9px;letter-spacing:4px;text-transform:uppercase;color:#4a2a3a;margin-bottom:16px">SOCIAL CREDIT SYSTEM — DIRECTIVE</p><h1 style="font-style:italic;font-size:32px;color:#b09898;letter-spacing:2px;margin-bottom:8px">☭ Reset Order ☭</h1><div style="width:60px;height:1px;background:#5a3a4a;margin:0 auto 28px"></div><p style="font-style:italic;font-size:14px;color:#8a6a6a;line-height:1.8;margin-bottom:28px">The Party acknowledges your request.<br>Your loyalty shall be restored.</p><a href="${link}" style="display:inline-block;background:linear-gradient(135deg,#2a1520,#1a0d18);border:1px solid #7a4a5e;color:#c87a9a;font-size:11px;letter-spacing:3px;text-transform:uppercase;text-decoration:none;padding:14px 32px;margin-bottom:28px">→ Restore Access ←</a><div style="width:60px;height:1px;background:#2a1520;margin:0 auto 24px"></div><p style="font-size:11px;color:#3a2a3a;line-height:1.7">This directive expires in one hour.<br>If you did not request this — the Party is already aware.</p></td></tr></table></td></tr></table></td></tr></table><tr><td align="center" style="padding:12px;border-top:1px solid #2a1520;background:#0a070a"><p style="font-style:italic;font-size:11px;color:#3a2a3a">☭ &nbsp; glory to the collective &nbsp; ☭</p></td></tr></table></td></tr></table></td></tr></table></body></html>`,
    })
    res.json({message:'If that email is registered, a reset link was sent.'})
  }catch(e){
    console.error('[mail]',e.message)
    res.status(500).json({error:'Email service unavailable: '+e.message})
  }
})
...
router.post('/reset-password',async(req,res)=>{
  try{
    ...
    user.password=password;user.resetToken=null;user.resetExpiry=null
    await user.save()
